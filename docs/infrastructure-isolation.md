...
The portal uses local Firebase emulators for development and one cloud project, `debageri-portal-prod`, for production. The local ID `debageri-portal-local` is deliberately fictitious and must never resolve to a cloud project. Production has its own Auth, Firestore, Storage, Cloud Run service, service accounts, IAM, secrets, App Check, logs, backups, CI environment, and billing/alerts. Nothing connects to `debageri-web-prod`; no credentials, exports, users, buckets, functions, or candidate records are reused.

`.firebaserc` contains only the `portal-prod` deployment alias and intentionally has no default. Deployment scripts explicitly select it, display it, and require the exact project ID `debageri-portal-prod`. Startup validation requires public/admin/expected IDs to match, refuses website IDs, requires the fictitious local ID plus emulators in local mode, requires the exact production ID in production, and refuses emulators in production.
