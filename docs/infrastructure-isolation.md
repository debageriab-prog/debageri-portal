...
Branch previews follow the website's tagged, zero-traffic Cloud Run revision
pattern and use the production runtime identity and Firebase configuration so
the complete workflow can be tested before merge. Consequently, branch code can
access production employee data. Repository write access, workflow changes,
preview URLs, and secrets require strict access control and review.
